...
if __name__ == '__main__':
    ...
    if choice_id:
        ...
        if class_list['rows'][choice_id]['Sfybm'] == '（未选）':
            ...
            # 验证结果
            print('正在验证选课是否成功...')
            print(check_class(user, class_id, class_list['rows'][choice_id]['Kxh'])[0])
        elif class_list['rows'][choice_id]['Sfybm'] == '【已选】':
            choice = input('检测到已选择，是否退课？(1：是 0：退出)：')
            if choice == '1':
                big_id = check_class(user, class_id, class_list['rows'][choice_id]['Kxh'])[1]
